...
export function RecentChatList() {
  ...
  const archivedThreads = useMemo(
    () => (infiniteArchivedThreads?.pages ?? []).flat(),
    [infiniteArchivedThreads?.pages],
  );
  const archivedAction = useThreadArchiveAction();
  const { mutate: deleteArchivedThread } = useDeleteThread();
                  ...
                  >
                    ...
                    {env.NEXT_PUBLIC_STATIC_WEBSITE_ONLY !== "true" && (
                      <DropdownMenu>
                        ...
                        >
                          <DropdownMenuItem
                            disabled={archivedAction.isPending}
                            onSelect={() =>
                              archivedAction.setArchived(
                                thread.thread_id,
                                false,
                              )
                            }
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem
                            onSelect={() => handleDeleteArchived(thread)}
                          >
                            <Trash2 className="text-muted-foreground" />
                            <span>{t.common.delete}</span>
                          </DropdownMenuItem>
