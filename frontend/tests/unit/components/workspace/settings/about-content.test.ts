...
test("aboutMarkdown heading interpolates the app version", async () => {
  ...
  expect(aboutMarkdown).toContain("[About quantflow 9.9.9-test]");
  // Milestone copy in the acknowledgments refers to the 1.0/2.0 product
  // generations and must NOT be parameterized.
  expect(aboutMarkdown).toContain("quantflow 1.0 and 2.0");
});
...
test("aboutMarkdown heading reflects the package version when env is unset", async () => {
  ...
  // empty or undefined APP_VERSION interpolation (`About quantflow ]` /
  // `About quantflow undefined]`), not just removal of the old literal.
  expect(aboutMarkdown).toContain(`[About quantflow ${APP_VERSION}]`);
});
